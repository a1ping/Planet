...
const petalPoints = [
  ...
  function drawFlower(x0, y0, size, color, lineWidth, flag) {
	scene.context.strokeStyle = color;
	scene.context.lineWidth = lineWidth;
	
	scene.context.beginPath();
	let alpa0, wk1, wk2, wk3, hk1, hk2, hk3;
	alpa0 = -0.435*Math.PI;
	if(flag==1)
	{
	  wk1=0.95;hk1=0.1;
  ...
  function drawMark()
  {   
	  let w, h, x0,y0,r,color, mx, my, nx, ny, ms, ns, range;
	  ms= 0.2*scene.height;
	  ns= 0.05*scene.height
	  range = ms - ns + 1;				
	  w = Math.floor((Math.random() * range) + ns);
	  h = w;
	  ...
	  nx = w/2;
	  range = mx - nx + 1;				
	  x0 = Math.floor((Math.random() * range) + nx);
	  my = scene.height-h/2;
	  ny = h/2;
	  range = my - ny + 1;				
	  y0 = Math.floor((Math.random() * range) + ny);
	  //x0 = w / 2;
	  //y0 = h / 2;
	  scene.context.globalAlpha = 1;
	  scene.context.lineWidth = 0.02*h;
	  r = 0.4*h;				
	  ...
	  scene.context.fill();
	  //draw petal outline
	  let size = Math.min(w, h) * 1.05 * 0.5;
	  color='rgba(0,0,0,1.0)';
	  drawFlower(x0, y0, size, color, 0.03*h, 1); 
	  //draw petal body
	  size = Math.min(w, h) * 0.95 * 0.5;
	  color='rgba(60,220,113,1.0)';
	  drawFlower(x0, y0, size, color, 0.04*h, 2);    
	  
	  //bottom_center_0
	  scene.context.lineWidth = 0.03*h;
	  scene.context.strokeStyle = 'rgba(0,0,0,1.0)';
	  scene.context.beginPath();
	  scene.context.arc(x0, y0+1.07*r, r*1.05, -2.8*Math.PI/4, -1.2*Math.PI/4, false);
	  scene.context.stroke();
  
	  scene.context.lineWidth = 0.04*h;
	  scene.context.strokeStyle = 'rgba(60,220,113,1.0)';
	  //bottom_center_1
	  scene.context.beginPath();
